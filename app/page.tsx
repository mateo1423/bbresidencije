...
const homes = [
  {n:"01",name:"Kuća Atrij",kind:"Prizemna rezidencija",area:"186 m²",place:"Pula, Istra",img:"/courtyard-bb.png",desc:"Privatnost organizirana oko središnjeg vrta i mirne vodene plohe."},
  {n:"02",name:"Vila Horizont",kind:"Obiteljska vila",area:"248 m²",place:"Ližnjan, Istra",img:"/hero-bb.png",desc:"Dugi kameni volumen otvara dnevne prostore prema moru i zalasku."},
  {n:"03",name:"Kuća Maslina",kind:"Kompaktna kuća",area:"142 m²",place:"Vodnjan, Istra",img:"/courtyard-bb.png",desc:"Suvremeni dom oblikovan oko postojeće masline i zaštićenog dvorišta."}
];
export default function Home(){
 const [open,setOpen]=useState(false); const [i,setI]=useState(0); const h=homes[i];
 const go=(d:number)=>setI((i+d+homes.length)%homes.length);
 return <main>
  <header className="nav"><a href="#top" className="brand"><b>BB</b><span>RESIDENCIJA</span></a><nav className={open?"open":""}><a href="#koncepti" onClick={()=>setOpen(false)}>Koncepti</a><a href="#ponuda" onClick={()=>setOpen(false)}>Aktualna ponuda</a><a href="#pristup" onClick={()=>setOpen(false)}>Pristup</a><a href="#proces" onClick={()=>setOpen(false)}>Proces</a><a href="#kontakt" className="cta" onClick={()=>setOpen(false)}>Kontakt ↗</a></nav><button onClick={()=>setOpen(!open)} aria-label="Izbornik"><i></i><i></i></button></header>
  <section className="hero" id="top"><img src="/hero-bb.png" alt="Suvremena istarska vila"/><div className="shade"></div><div className="heroCopy"><Label light>Arhitektura za život</Label><h1>Prostor koji<br/><em>ostaje.</em></h1><p>Autorski koncepti suvremenih kuća ukorijenjenih u istarski krajolik.</p></div><a className="down" href="#uvod">↓ &nbsp; OTKRIJTE</a><span className="year">BB / 2026</span></section>
  ...
  <footer><div className="brand"><b>BB</b><span>RESIDENCIJA</span></div><p>Pula · Istra · Hrvatska</p><p>© 2026 BB Residencija</p></footer>
 </main>
}
function Label({children,light=false}:{children:React.ReactNode,light?:boolean}){return <p className={light?"label light":"label"}><i></i>{children}</p>}
function Principle({n,title,children}:{n:string,title:string,children:React.ReactNode}){return <article><small>{n}</small><h3>{title}</h3><p>{children}</p></article>}
